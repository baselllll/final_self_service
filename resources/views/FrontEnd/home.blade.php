                    ...
                    <div style=" overflow: auto;">
                        <table id="absense_table" class="table table-striped bg-dark" style="width:100%; color: white">
                            ...
                            <tr>
                                ...
                                <th>@lang('messages.TransitionId')</th>
                                    ...
                                    @else
                                        @if(session()->get("user_type") == "employee" and $absence->approval_status == $Pend_approved_pending_req)
                                            <tr>
                                                ...
                                                <td style="color: white; font-size: 15px;">{{$absence->transaction_id }}</td>
                                                @if(session()->get('user_type') != "employee")
                                                    <td  style="color: white; font-size: 15px;">{{$absence->empno}}</td>
                                                    <td style="color: white; font-size: 15px;">{{explode(' ',$absence->emp_name)[0] . ' ' . explode(' ',$absence->emp_name)[1]}}</td>
                                                ...
                                                <td style="color: white; font-size: 15px;">{{\Carbon\Carbon::parse($absence->creation_date)->format('Y-m-d H:i')}}</td>
                                                <td style="color: white; font-size: 15px;">@if(str_contains($absence->approval_status,"Rejected")) Rejected @elseif($absence->approval_status=="Admin Mgr Approved" and $absence->no_of_approvals=="2" ) Approved @else {{$absence->approval_status}} @endif</td>
        ...
        setTimeout(function() {
            document.getElementById('spinner').style.display = 'none'; // Hide the spinner
             @if(env('APP_URL')=="http://192.168.203.64")
                 window.location.href = "/portal-services";
             @else
                 window.location.href = "/public/index.php/portal-services";
             @endif
