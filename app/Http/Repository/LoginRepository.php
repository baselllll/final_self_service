...
//        }
                            ...
                            if (count($data_phone) > 0) {
                                ...
                                if (strlen($cur_zero_number) == 12) {
                                    $result = $this->smsVerifyHelper->sendSMS(
                                        $cur_zero_number,
                                         trans('messages.OtpSms_Message_after'),$otp
                                    );
                                }
                            }
    ...
    public function storeDelegate($data){
        ...
        try {
            ...
            DB::table('SELFSERVICE.XXAJMI_DELEGATE_REQUESTS')->insert([
            ...
            DB::statement("BEGIN SELFSERVICE.xxajmi_delegation_notif($delegate_id); END;");
            return "true";
        }catch (\Exception $exception){
...
JOIN apps.fnd_flex_values ffv ON ffv.flex_value = pac.segment1
