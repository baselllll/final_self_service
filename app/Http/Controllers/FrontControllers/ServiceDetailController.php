...
//           }
        ...
        if(isset($lastRecord)){
            if($absence_attendance_type_id ==AppKeysProps::Sickleave_absence_type_id()->value
                or (($absence_attendance_type_id==AppKeysProps::Emergency_absence_type_id()->value))
                or (($absence_attendance_type_id==AppKeysProps::Annual_Leave()->value))
                or (($absence_attendance_type_id==AppKeysProps::PersonnalPremission_absence_type_id()->value))
                or (($absence_attendance_type_id==AppKeysProps::WorkPremission_absence_type_id()->value))
            ){
    ...
    public function CalculateAccruals(Request $request)
    {
        ...
        if (isset($start_date)){
            $originalDate = Carbon::parse($start_date);
    ...
    public function AddServiceDetail(Request $request){
        ...
        if (preg_match($datePattern, $start_date_absence) && preg_match($datePattern, $end_date_absence)){
            ...
            [$datePart_start_date, $timePart_start_date] = explode(' ', $start_date_absence);
            [$datePart_end_date, $timePart_end_date] = explode(' ', $end_date_absence);
        ...
        if (isset($lastRecordSameService[0])){
            if($lastRecordSameService[0]->absence_type ==AppKeysProps::Sick_Leave()->value
                or $lastRecordSameService[0]->absence_type==AppKeysProps::Emergency_Leave()->value
                or $lastRecordSameService[0]->absence_type==AppKeysProps::Permit_Req()->value
                ){
        ...
        $GetSick_EmP_AnnualFromCustomNotifyWF = $this->detailsEmployeeService->GetEmerg_EmPFromCustomNotifyWF($employee->employee_number);
        $GetAnnualFromCustomNotifyWF = $this->detailsEmployeeService->GetAnnualFromCustomNotifyWF($employee->employee_number);
        $GetSickFromCustomNotifyWF = $this->detailsEmployeeService->GetSickFromCustomNotifyWF($employee->employee_number);
        ...
        $GetAnnualFromCustomNotifyWF = $this->detailsEmployeeService->GetAnnualFromCustomNotifyWF($employee->employee_number);
        $GetSickFromCustomNotifyWF = $this->detailsEmployeeService->GetSickFromCustomNotifyWF($employee->employee_number);
        $checked_overlap  = $this->specialSpecifService->checkOverlap($start_date_absence,$end_date_absence);
        if(isset($GetSick_EmP_AnnualFromCustomNotifyWF) or isset($GetAnnualFromCustomNotifyWF) or isset($GetSickFromCustomNotifyWF)){

        } else{
            if($checked_overlap=='overlapped'){
                Alert::warning(__('messages.added_service_validate1_title'),__('messages.added_service_validate1_message'));
                return redirect()->to('profile-employee');
            }
        }
...
//                }
//                if ($lastRecord->no_of_approvals== 3 and ($lastRecord->approval_status == "Pending Approval" or $lastRecord->approval_status== "Manager Approved" or $lastRecord->approval_status== "Admin Mgr Approved")){
//                    if($lastRecord->absence_type== AppKeysProps::Emergency_Leave()->value or $lastRecord->absence_type== AppKeysProps::Annual_Leave()->value ){
//
//                    }else{
//                        Alert::warning(__('messages.added_service_validate1_title'),__('messages.added_service_validate1_message'));
//                        return redirect()->to('profile-employee');
...
//
//                if ($lastRecord->no_of_approvals== 2 and ($lastRecord->approval_status == "Pending Approval" or $lastRecord->approval_status== "Manager Approved")){
//                    if($lastRecord->absence_type== AppKeysProps::Sick_Leave()->value  ){
//
//                    }else{
//                        Alert::warning(__('messages.added_service_validate1_title'),__('messages.added_service_validate1_message'));
//                        return redirect()->to('profile-employee');
...
////        }
        ...
        $comments = $request->official_permit_comments;
        ...
        $difference_hours =  number_format($request->difference_hours, 2);
