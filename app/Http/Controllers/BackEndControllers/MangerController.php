...
class MangerController extends Controller
{
    ...
    public function ApproveRequest(Request $request){
        try {
            ...
            $xxajmi_notif = DB::table("xxajmi_notif")->select('*')->where('transaction_id', '=', $request->transaction_id)->first();
            $absence_type = $xxajmi_notif->absence_type;
            $replacement_no = $request->replacement_employee_number;
            if($xxajmi_notif->service_type=='eos'){

            }else{
                if($user_type == AppKeysProps::Manger()->value and !isset($replacement_no) and ($absence_type==AppKeysProps::Annual_Leave()->value or $absence_type==AppKeysProps::Emergency_Leave()->value )){
                    Alert::error("ERROR",__('messages.detectReplacement'));
                    return redirect()->route('profile-employee',['user_type'=>$user_type]);
                }
            }

//            $check_spayroll =  $this->mangerLogicService->xxajmi_leave_prc_next_payroll($request->transaction_id)[0]->stop_payroll;
//            if($check_spayroll=="Y"){
//                Alert::error("ERROR",__('messages.NotAllowed'));
//            }
            $this->mangerLogicService->ApproveRequest($request->notify_confirm,$request->transaction_id,$request->note,$request->type_person,$replacement_no);
    ...
    public function RejectRequest(Request $request){
        try {
            ...
            Alert::warning("Rejected",__('messages.mr_Reject'));
            return redirect()->route('profile-employee');
        }catch (\Exception $exception){
