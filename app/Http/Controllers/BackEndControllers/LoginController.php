...
class LoginController extends Controller
{
    ...
    public  function Login(LoginRequest $request)
    {
        ...
        if($currentDomain=="192.168.203.64"){
            $test_otp_as_dev = '2810972023';
        }else{
            $test_otp_as_dev=null;
        }
    ...
    public function index(){
        ...
        DB::table("sshr_new_features")->update(['no_of_hits'=>++$feature_new->no_of_hits]);
        $number_of_hits = $feature_new->no_of_hits ;
        if(isset(session()->get('employee')->person_id)){
            Alert::success("SUCCESS",__('messages.logined_now'));
            return redirect('home');
        }
        return view('frontend.login',compact('feature_new','number_of_hits'));
    }
    ...
    public function send_otp_for_check_otp(Request $request){
        ...
        if(isset($employee_full_data->attribute6) and $employee_full_data->attribute6==$request->otp and $expire_date > Carbon::now()) {
            ...
            $expire_date = $expire_date->subMinutes(2);
            $this->loginService->update_per_people_attr(null, $expire_date->subMinutes(2),$request->employee_number);
...
//                }
    ...
    public function GetEmployeeInformation(){
        ...
        $employeeNumber = $employee->employee_number;  // Replace this with the actual employee number
       $imagePattern = "image_profile*";  // The asterisk (*) acts as a wildcard to match any characters after "image_profile"

        $folderPath = "documents/$employeeNumber";
        $filePathPattern = $folderPath . '/' . $imagePattern;

// Find the matching file using glob
        $matchingFiles = glob(public_path($filePathPattern));
        $relativePath=null;
// Check if any matching files were found
        if (!empty($matchingFiles)) {
            ...
            // Extract the relative path from public_path()
            $relativePath = str_replace(public_path(), '', $firstMatchingFile);
        }
    ...
    $filePath = $relativePath;
        ...
        return view('frontend.employee-information',compact('employee','filePath','cost_center','employee_more_details','requested_notification'));
...
// Assuming $record_approved->empno, $file_output_pdf_path, and $file_output_pdf are defined elsewhere
    ...
    public function update_phone_profile(Request $request){
        try {
            $this->loginService->update_phone_profile($request->employee_number,$request->phone_number);
            return response()->json(['verified'=>'1']);
        }catch (\Exception $exception){
            return response()->json(['verified'=>'0']);
        }
