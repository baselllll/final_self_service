...
class NotifyMangerService
{

    public function sendsms($name, $emp_requested_number, $absence_type, $transaction_id, $phone_number,$email_address)
    {
        ...
        try {
            ...
            if(isset($email_address)){
                // DB::statement("BEGIN selfservice.xxajmi_send_msg_email_direct('$email_address','$message'); END;");
            }
...
");
    ...
    public function handle()
    {
        ...
        if (count($manger_items) > 0) {
            foreach ($manger_items as $item){
                ...
                $this->sendsms($name, $emp_requested_number, $absence_type, $transaction_id, $phone_number,$email_address);
                try {
            ...
            foreach ($admin_manger_items as $item){
            ...
            foreach ($top_manger_items as $item){
    ...
    public function handeloneByone($transaction_id){
       $item =  DB::table('xxajmi_notif')
            ->where('transaction_id', $transaction_id)->first();
        if($item->approval_status=="Pending Approval" and $item->times_call == 0){
                $data = $this->getPhone($item->mgr_person_id);
        ...
        if($item->approval_status=="Manager Approved" and $item->times_call == 0){
                $data = $this->getPhone($item->admin_mgr_person_id);
                $phone_number = $data->phone_number;
                $name = $data->full_name;
                $email_address = $data->email_address;
        ...
        if($item->approval_status=="Admin Mgr Approved" and $item->times_call == 0){
                $data = $this->getPhone($item->top_mgmt_person_id);
                $phone_number = $data->phone_number;
                $name = $data->full_name;
                $email_address = $data->email_address;
